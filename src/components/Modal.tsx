...
const Modal: React.FC<ModalProps> = ({ children, onClose }) => {
  return (
    ...
    >
      <div
        className="bg-white dark:bg-stone-700 rounded-lg shadow-lg w-full max-w-md p-6 relative"
        onClick={(e) => e.stopPropagation()} // Предотвращение закрытия при клике внутри
      >
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-2xl text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-100"
          aria-label="Закрыть"
        >
          ×
        </button>
